...
class AverageFloor2020Spec extends DataCorrectnessSpec {

  override def expectedNbOfRows: Long = 26

  override val datasetInterface: DatasetInterface = AverageFloor2020()
  ...
  "toMap" should "contain entries with correct values for NE, TI, BE" in {
    val avgFloorMap = AverageFloor2020().toMap
    // avgFloorMap.toList.sortBy{case (canton, floor) => canton}.foreach(println)
    assertResult(1.46491536812466)(avgFloorMap("NE"))
    assertResult(1.174974680620458)(avgFloorMap("TI"))
    assertResult(1.265527085951403)(avgFloorMap("BE"))
  }

  // "writeToFile" should "write to the correct file (MANUAL CHECK)" in {
  //   AverageFloor2020().writeToFile
  // }
}
