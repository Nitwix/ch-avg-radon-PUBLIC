...
abstract class AverageFloor(
    datasetPath: String,
    schema: StructType,
    options: DatasetInterface.DataFrameReaderOptions
)(implicit spark:SparkSession) extends DatasetInterface(datasetPath, schema, options){
  lazy val toMap: Map[String, Double] = {
    df
      .collect()
      .map((row: Row) =>
        (row.getAs[String](s.canton), row.getAs[Double](s.floor))
      )
      .toMap
  }
}
